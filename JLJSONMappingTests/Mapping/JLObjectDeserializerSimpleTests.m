...
- (void)testPropertyNameMapping
{
    ...
    NSLog(@"%@", objectJSON);
    SimpleTestMappingObject *objectFromString = [deserializer objectWithString:objectJSON targetClass:[SimpleTestMappingObject class] error:NULL];
...
- (void)testDictionaryJLDeserializerErrorOnAmbiguousTypeDontError
{
    ...
    NSError *error;
    [deserializer objectWithString:jsonWithExtraProperty targetClass:[ObjectAmbiguousType class] error:&error];
    XCTAssertNil(error);
}
...
- (void)testJsonEscapedFeatureStillApplies
{
    ...
    NSError *error;
    [deserializer objectWithString:stringThatCrashes targetClass:[HTMLContainingObject class] error:&error];
...
{
    NSString *stringThatCrashes  = @"{\"html\":\"<div><a href=\"http://www.google.com.com\" target=\"_blank\" data-mce-href=\"http://www.google.com\">google.com</a></div><div> 1:21 PM 7/6/14 is this right.</div><div><ol><li>something something yeah ok</li><li>bullets are cool.</li><li>Yet another bullet.</li></ol></div>\"}";
    HTMLContainingObject *crasher = [[HTMLContainingObject alloc] init];
    crasher.html = stringThatCrashes;
    NSDictionary *data = [serializer JSONObjectWithObject:crasher];
    
    NSError *error;
    [deserializer objectWithJSONObject:data targetClass:[HTMLContainingObject class] error:&error];
    XCTAssertEqual(error.code, JLDeserializationErrorInvalidJSON, @"This shouldn't have worked, either the fix is not needed, or something is whack");
}
...
- (void)testPassingObjectToDeserializerExpectingString
{
    ...
    NSDictionary *data = [serializer JSONObjectWithObject:crasher];
    NSError *error;
    [deserializer objectWithString:(NSString *)data targetClass:[HTMLContainingObject class] error:&error];
...
- (void)testJsonEscapedFeature
{
    ...
    HTMLContainingObject *newObject = [deserializer objectWithString:data targetClass:[HTMLContainingObject class] error:NULL];
